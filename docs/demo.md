...
What the execution layer should do next: for a canonical-action pilot response, rebuild and validate the exact final intent, verify the receipt, validate and consume the short-lived execution authorization, then execute the lookup and store the DecisionReceipt. A legacy compatibility response without execution authorization is not an executable authorization.
...
- `allow`: verify the receipt, rebuild the final intent, validate `execution_intent_digest`, TTL, and single-use authorization, then execute and store the DecisionReceipt.
