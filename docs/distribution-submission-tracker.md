...
| MCP Registry | https://registry.modelcontextprotocol.io | MCP ecosystem | P0 | `prepared` | Yes | GitHub namespace authentication required | Remote `server.json` publish flow | Manual first, automatable later | `server.json`, `distribution/mcp-readiness.md` | Authenticate as an owner/member authorized for `InterAILabs`, publish, then verify with the Registry API. | 2026-08-25 | `server.json` passed `mcp-publisher v1.8.1 validate`; official client v2.0.0 completed handshake, tools/list, and tool call. Not listed yet. |
