import { VerifyInput, VerifyOutput } from "./types.js"
...
export function scoreResponse(input: VerifyInput): VerifyOutput {
...
if (looksLikeMathPrompt(prompt) && responseHasNumber(response)) {
  ...
  if (numericalConflict) consistency -= 0.2
  if (absoluteClaims > 2) consistency -= 0.1
  ...
  if (hallucinationRisk > 0.6) riskLevel = "high"
  else if (hallucinationRisk > 0.3) riskLevel = "medium"

  let action: "accept" | "review" | "reject" = "accept"
  if (riskLevel === "medium") action = "review"
  if (riskLevel === "high") action = "reject"
