...
export const verifyBatchRoute = async (app) => {
    app.post("/verify/batch", async (req, reply) => {
        ...
        const results = body.items.map((item) => scoreResponse({
            prompt: item.prompt ?? "",
            response: item.response ?? "",
            domain: item.domain ?? "general"
        }));
        const avgConsistency = results.reduce((acc, item) => acc + item.consistency_score, 0) / results.length;
