...
| robots.txt | `implemented_local` | Route serves crawler policy and sitemap URL after this change. | https://api.interailabs.dev/robots.txt | Deploy after validation. | no | 200 text/plain in production. | discovery.bundle.view | Blocking useful routes by accident. | high |
| sitemap.xml | `implemented_local` | Route lists public discovery, demo, pricing, OpenAPI, and manifests. | https://api.interailabs.dev/sitemap.xml | Deploy after validation. | no | 200 application/xml in production and parseable XML. | discovery.bundle.view | Stale lastmod if generated incorrectly. | high |
| llms.txt | `implemented_local` | Route summarizes demo, pricing, OpenAPI, manifests, and safety limits. | https://api.interailabs.dev/llms.txt | Deploy after validation. | no | 200 text/plain in production. | demo_started | Over-claiming capabilities. | high |
| .well-known manifests | `live_needs_demo_alignment` | ai-service, discovery bundle, agent card, autonomous adoption are live JSON. | https://api.interailabs.dev/.well-known/ai-service.json | Deploy demo links and web discovery links. | no | Manifests mention `/demo`, `/pricing`, OpenAPI, receipts, and no approval claims. | discovery.ai_service.view | Metadata drift from live routes. | high |
...
| A2A | `hosted_surface_live` | A2A endpoint and agent card are live discovery surfaces. | https://api.interailabs.dev/.well-known/agent.json | Add more examples and conformance notes later. | no | Agent card stays valid and examples run. | discovery.agent_card.view | Experimental protocol drift. | medium |
